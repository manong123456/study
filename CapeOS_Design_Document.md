...
│ 系统监控     │ │ JWKS端点     │ │ MergerFS   │ │ 容器生命周期 │ │                  │
│ Samba共享    │ │ 账号管理      │ │ 分区管理    │ │ (bollard)  │ │  REST + WS       │
│ 云存储挂载   │ │ (argon2)     │ │            │ │            │ │  + Unix Socket   │
│ ZeroTier     │ │              │ │            │ │            │ │                  │
│              │ │              │ │            │ │            │ │                  │
...
| CPU 温度 | `get_cpu_temperature()` | `sysinfo::Components` 或读取 `/sys/devices/virtual/thermal/thermal_zone*/temp` |
| CPU 功耗 | `get_cpu_power()` | `tokio::fs::read_to_string("/sys/class/powercap/intel-rapl/*/energy_uj")` |
...
| 主机信息 | `get_sys_info()` | `sysinfo::System::host_name()` / `os_version()` 等 |
...
| 目录操作 | `get_dir_path()` / `mkdir_all()` / `rename_file()` | `tokio::fs` 标准异步文件操作 |
...
| 分块上传 | 追踪上传状态，写入 `.tmp` 文件，完成后重命名 | `axum::extract::Multipart` + `DashMap` 追踪状态 |
| 文件下载 | 单文件流式响应；批量打包下载 | `axum::body::Body::from_stream()` + `zip`/`tar` crate |
...
| 复制/移动 | 异步执行，进度追踪，支持取消 | `tokio::spawn` + `CancellationToken` + 进度通知 |
...
**驱动 trait** (定义在 `crates/capeos-main/src/server/drivers/mod.rs`):
...
**健康检查模块 (HealthService)**: 通过 `zbus_systemd` 列出所有 `capeos-*` 服务运行状态；通过读取 `/proc/net/tcp` 和 `/proc/net/udp` 获取 TCP/UDP 端口占用；打包日志下载（`zip` crate）。
...
| USB 管理 | 列出 USB 存储设备，自动挂载 | 读取 `/sys/block/` + `udev` 或 `tokio::process` |
...
│   │   ├── Cargo.toml                  # deps: axum, axum-reverse-proxy, capeos-common
│   │   └── src/
...
│   │   ├── Cargo.toml                  # deps: axum, tokio, capeos-common
│   │   └── src/
│   │       ├── main.rs
...
│   │   ├── Cargo.toml                  # deps: axum, rusqlite, jsonwebtoken, p256, argon2
│   │   └── src/
│   │       ├── main.rs
...
│   │   ├── Cargo.toml                  # deps: axum, sysinfo, tokio, capeos-common
│   │   └── src/
│   │       ├── main.rs
...
│   │   ├── Cargo.toml                  # deps: axum, bollard, rusqlite, capeos-common
│   │   └── src/
│   │       ├── main.rs
...
│   │   ├── Cargo.toml                  # deps: leptos, leptos_axum, axum, rusqlite, sysinfo...
│   │   ├── src/
│   │   │   ├── main.rs                 # Leptos + Axum 启动入口
│   │   │   ├── app.rs                  # Leptos 根组件 + 前端路由
│   │   │   │
│   │   │   ├── server/                 # === 后端 (仅服务端编译, #[cfg(feature="ssr")]) ===
│   │   │   │   ├── mod.rs
...
│   │   │   │   │   ├── upload.rs       # 分块上传 (DashMap 追踪)
│   │   │   │   │   ├── shares.rs       # Samba 共享 (rusqlite)
│   │   │   │   │   ├── connections.rs  # 网络连接 (rusqlite + pavao)
│   │   │   │   │   ├── storage.rs      # 云存储 (rclone via tokio::process)
│   │   │   │   │   ├── notify.rs       # 通知 (rusqlite + MessageBus)
│   │   │   │   │   ├── health.rs       # 健康检查 (zbus_systemd)
│   │   │   │   │   └── peer.rs         # 设备发现 (rusqlite)
│   │   │   │   │
│   │   │   │   ├── repo/               # Repository 层 (rusqlite 数据访问)
│   │   │   │   │   ├── mod.rs
│   │   │   │   │   ├── shares_repo.rs
│   │   │   │   │   ├── connections_repo.rs
│   │   │   │   │   ├── notify_repo.rs
│   │   │   │   │   └── peer_repo.rs
│   │   │   │   │
│   │   │   │   ├── api/                # REST API (给第三方 / 非 Leptos 客户端)
│   │   │   │   │   ├── mod.rs
│   │   │   │   │   └── v1.rs           # Axum handlers + utoipa 标注
│   │   │   │   │
│   │   │   │   └── drivers/            # 云存储驱动
...
│   │   │   │       └── dropbox.rs
│   │   │   │
│   │   │   ├── pages/                  # === 前端页面 (Leptos WASM 组件) ===
│   │   │   │   ├── mod.rs
│   │   │   │   ├── dashboard.rs        # 仪表盘 (CPU/内存/磁盘/网络)
│   │   │   │   ├── files.rs            # 文件管理器
│   │   │   │   ├── apps.rs             # 应用商店
│   │   │   │   ├── storage.rs          # 存储管理
│   │   │   │   ├── shares.rs           # 网络共享
│   │   │   │   ├── settings.rs         # 系统设置
│   │   │   │   ├── terminal.rs         # SSH WebSocket 终端
│   │   │   │   └── login.rs            # 登录页
│   │   │   │
│   │   │   └── components/             # === 可复用 UI 组件 ===
│   │   │       ├── mod.rs
│   │   │       ├── layout.rs           # 页面布局 (侧边栏 + 顶栏)
│   │   │       ├── file_list.rs        # 文件列表/网格视图
│   │   │       ├── file_uploader.rs    # 拖拽上传组件
...
│       ├── Cargo.toml                  # deps: clap, reqwest, capeos-common
│       └── src/
│           └── main.rs
│
├── config/
...
| 迁移 | `rusqlite_migration` (Apache-2.0) | 利用 SQLite `user_version` pragma，无额外表 |

设计原则：结构化数据设计，少冗余，少磁盘 IO，所有结构化数据用表存储，不使用文件存储。

### 7.2 数据库初始化

数据库初始化在 `crates/capeos-main/src/server/db.rs` 中实现：
...
**关键设计点**:
- **WAL 模式**: 提升并发读写性能
- **基于 user_version 的迁移**: `rusqlite_migration` 利用 SQLite 内置 `PRAGMA user_version`，无需额外迁移追踪表
- **异步封装**: `tokio-rusqlite` 在后台线程执行所有 SQL，通过 channel 与 Tokio 运行时交互，避免阻塞异步任务
...
CREATE TABLE IF NOT EXISTS o_notify (
    ...
    name      TEXT,
    icon      TEXT,
    state     INTEGER DEFAULT 0,  -- 0=变动中未读, 1=未读, 2=已读
...
| 存储方式 | 用途 | 位置 |
|----------|------|------|
| **SQLite (rusqlite)** | 所有结构化数据：通知、共享、连接、设备、配置 | `/var/lib/capeos/db/capeos.db` |
| **TOML 配置文件** | 应用启动配置 | `/etc/capeos/capeos.toml` |
| **内存缓存 (moka)** | 运行时高频读取：CPU 温度路径、版本信息 | 进程内存 |
